...
export function useFinanceLifecycle(input: UseFinanceLifecycleInput) {
  ...
  const submitClaim = useCallback(() => {
    if (!acceptanceReady) {
      return {
        ok: false as const,
        message: '[#A15] Acceptance=OK ?ä??¯é€å‡ºè«‹æ¬¾??,
      };
    }
    ...
    if (lineItems.length === 0) {
      return {
        ok: false as const,
        message: '[#A15] Claim Preparation ?€?³å?ä¸€?‹å‹¾?¸é??®ä? quantity > 0??,
      };
    }
  ...
  const completePayment = useCallback(() => {
    if (stage !== 'payment-term') {
      throw new Error('[#A16] ç¦æ­¢è·³é??Ÿå‘½?±æ?æ­¥é??´æŽ¥ç¢ºè??¶æ¬¾??);
    }

    if (currentClaimLineItems.length === 0) {
      throw new Error('[#A16] ?€?ˆå???Claim/Invoice/PaymentTerm ä¸¦å…·?™æ??ˆè?æ¬¾é??®ã€?);
    }
  ...
  const closeCycle = useCallback(() => {
    if (stage !== 'payment-received') {
      throw new Error('[#A16] ?€??Payment Received ?Žæ®µ?èƒ½?œé??¬è¼ªè«‹æ¬¾??);
    }
