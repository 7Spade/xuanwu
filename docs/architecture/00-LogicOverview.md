...
%%    Path Map: VS0=src/shared-kernel + src/shared-infra/frontend-firebase
...
%%      src/shared-infra/frontend-firebase  = VS0-Infra（L7 Adapter 層）
%%      src/features/infra.gateway-query   = L6 Query Gateway（非業務切片）
%%      src/features/projection.bus        = L5 Projection Bus（非業務切片）
%%      src/shared-kernel/observability    = VS0-Kernel（L9 Observability）
%%    命名規則：VS0=Foundation Index（L1+L7+L9）；VS1~VS8=業務切片編號（L3）
%%    VS0 識別規格（文件/審查一律使用）:
%%      VS0-Kernel = src/shared-kernel/*（pure contracts/constants/functions，禁止 I/O）
%%      VS0-Infra  = src/shared-infra/frontend-firebase/*（SDK adapters only）
...
%%    ※ VS0(Foundation) 不屬於 L3 Domain Slices；其中 VS0-Kernel=L1，VS0-Infra=L7
%%    ※ 邊界澄清：VS0-Kernel=L1+L9（契約+可觀測）；VS0-Infra=L7（Adapter）；L6/L8 為 Shared Infrastructure Plane（執行層）
%%  ── 標準目錄結構（Standard Directory Structure · 單向依賴鏈對齊）──
%%    src/
%%      shared-kernel/                          # VS0-Kernel: contracts/constants/pure zone（L1）+ observability subtree（L9）
%%      shared-kernel/observability/            # VS0-Kernel / L9: metrics/errors/trace observability
...
%%  ── Folder Placement Decision Matrix（新增檔案放置決策）──
%%    Step 1: 先判斷「是否有 I/O / side effects」
%%      - 無 I/O（純型別/常數/純函式）→ src/shared-kernel/*（VS0-Kernel / L1）
%%      - 有 I/O → 進 Step 2
%%    Step 2: 判斷「是否 Firebase SDK 呼叫」
%%      - 是 → src/shared-infra/frontend-firebase/*（VS0-Infra / L7 Adapter）
%%      - 否 → 進 Step 3
%%    Step 3: 判斷「是否切片核心業務規則」
%%      - 是（aggregate / domain policy / business invariant）→ src/features/{slice}.slice/*（L3）
%%      - 否 → 進 Step 4
%%    Step 4: 判斷「是寫入協調、事件路由、投影、讀出口、可觀測哪一層」
%%      - Command orchestration / guards → src/features/infra.gateway-command（L2）
%%      - Event routing / relay / DLQ     → src/features/infra.event-router | infra.outbox-relay | infra.dlq-manager（L4）
%%      - Projection materialization       → src/features/projection.bus（L5）
%%      - Query/read registry              → src/features/infra.gateway-query（L6）
%%      - Observability                    → src/shared-kernel/observability（VS0-Kernel / L9）
%%    Step 5: 判斷「是否跨切片權威出口」
%%      - 搜尋權威 → src/features/global-search.slice
%%      - 副作用權威（通知）→ src/features/notification-hub.slice
%%    速記原則：Pure→L1，SDK→L7，Business→L3，Pipeline→L2/L4/L5/L6/L9，Authority Exit→cross-cut slices
%%  ── 依賴方向約束（對應目錄）──
...
%%  ── Scope（本輪必審）──
%%    1) VS0~VS8：每個編號域必須有明確層位與單一職責（VS0=L1+L7+L9；VS1~VS8=L3）
...
subgraph SHARED_INFRA_PLANE["🧩 Shared Infrastructure Plane（Execution Plane：L6/L8；含 VS0-Infra(L7) Adapter）"]
        direction TB

        subgraph GW_QUERY["🟢 L6 · Query Gateway（src/features/infra.gateway-query）[S2 S3]"]
            direction LR
        ...
        subgraph OBS_LAYER["⬜ L9 · Observability（src/shared-kernel/observability）"]
            direction LR
...
%%  D8  shared-kernel/*（except src/shared-kernel/observability/*）禁止 async functions、Firestore calls、side effects
