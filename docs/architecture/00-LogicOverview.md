...
%%    MUST: IF 涉及全域語義註冊 THEN 必須在 VS8 Core Domain（CTA/tag-definitions）定義；IF 涉及組織任務/技能標籤 THEN 必須在 VS4 org-tag-registry 定義
...
%%    任務語義與成本決策由 VS8 提供全域基線；組織自訂任務/技能標籤必須經 VS4 治理並投影到 tag-snapshot。
...
%%    新規則先索引、再實作；優先引用現有契約；全域語義進 VS8 註冊，組織任務/技能語義進 VS4 註冊；
...
%%    [D21-A] 雙層註冊律：全域語義在 VS8 core/tag-definitions.ts；組織任務/技能標籤在 VS4 org-tag-registry 註冊
...
%%    [D21-7] 語義讀取必須經由 projection.tag-snapshot；寫入必須經 CMD_GWAY 進入 VS8 CTA（全域）或 VS4 org-tag-registry（組織）
...
%%    業務切片（VS1~VS6，除 VS4 org-tag-registry）禁止私自宣告語義類別；組織自訂任務/技能標籤必須透過 VS4 治理流程 [D21-1]
...
subgraph VS4["🟧 VS4 · Organization Slice（src/features/organization.slice）"]
    ...
    subgraph VS4_TAG["🏷️ Tag 組織作用域（src/features/organization.slice）[S4]"]
        ...
        ORG_TAG_REG["org-tag-registry.aggregate\n組織可新建/編輯標籤（task/skill）\n命名空間：org:{orgId}:task:* / org:{orgId}:skill:*"]
        SKILL_POOL[("skill-tag-pool\n組織作用域技能標籤（可寫 Overlay）\n[S4: TAG_MAX_STALENESS ≤ 30s]")]
        TASK_POOL[("task-tag-pool\n組織作用域任務標籤（可寫 Overlay）\n[S4: TAG_MAX_STALENESS ≤ 30s]")]
...
%%  #A6  全域語義權威 = VS8 CENTRALIZED_TAG_AGGREGATE；組織任務/技能擴展權威 = VS4 org-tag-registry [D21-1]
...
%%  #A18 Org Tag Extension contract：組織可新建 task/skill 標籤；必須走 VS4 org-tag-registry，並以 org namespace 寫入 tag-snapshot
%%  ╠══════════════════════════════════════════════════════════════════════════╣
%%  TAG SEMANTICS 擴展規則（VS8 · 四層語義引擎擴展規則 [D21-1~D21-10 + D21-A~D21-X]）
%%  T1  新切片訂閱 TagLifecycleEvent（BACKGROUND_LANE）即可擴展 [D21-6]
%%  T2  SKILL_TAG_POOL / TASK_TAG_POOL = 組織作用域可寫 Overlay（來源：VS8 全域 + VS4 組織自訂）
...
%%  T8  組織新建標籤僅限 task/skill 類別，且必須使用 org namespace tagSlug，避免污染全域語義空間
%%  ╠══════════════════════════════════════════════════════════════════════════╣
...
%%  D21-1 語義唯一性（雙層）：全域語義類別與標籤實體由 VS8 CTA 定義；組織自訂 task/skill 標籤由 VS4 org-tag-registry 定義
...
%%  D21-7 讀寫分離原則：寫入操作必須經過 CMD_GWAY 進入 VS8 CTA（全域）或 VS4 org-tag-registry（組織）；讀取嚴禁直連資料庫，必須經由 projection.tag-snapshot
...
%%  D21-A 雙層註冊律：跨領域全域概念在 core/tag-definitions.ts 註冊；組織任務/技能概念在 VS4 org-tag-registry 註冊
