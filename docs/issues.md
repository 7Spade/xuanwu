...
> **Last verified**: 2026-03-02 — 8 open issues carried forward; 3 new issues added (ARCH-D5-002, ARCH-D5-003, ARCH-D3-003); 8 fixed entries removed  
...
> **Status**: 11 issues open (ARCH-D5-002 ⭐ ARCH-D5-003 ⭐ ARCH-D3-003 ⭐)
...
| ARCH (D3 Mutation Location) | 0 | 3 | 0 | 3 |
| ARCH (D5 UI Infra Boundary) ⭐ new | 0 | 2 | 0 | 2 |
| ARCH (D8 Shared-Kernel Purity) | 0 | 1 | 0 | 1 |
| TYPE (D19 Contract) | 0 | 2 | 0 | 2 |
| **Total** | **0** | **9** | **2** | **11** |
...
**Severity**: Major (D3 + D5 dual violation)  
**Rules**:
...
| P5 | ARCH-D3-002 | Move demand-board projector to `projection.bus/` or delegate via `_actions.ts` | Fixes D3 for domain-slice projector |
| P6 | ARCH-D6-001 | Amend D6 rule to allow `_hooks/` client boundary OR wrap all hooks in `_components/` | Resolves 20-file systematic D6 gap |
| P7 | TYPE-D19-001 | Move 3 type defs + update 1 file | Fixes ownership inversion for skill contracts |
| P8 | TYPE-D19-002 | Phase 1 only: promote `ScheduleItem`/`ScheduleStatus` | Highest-leverage cross-BC contract migration |
