---
title: Logic Overview v6 — High-Efficiency Consistent Runtime Architecture
---
...
%% ==========================================================================
%% LOGIC OVERVIEW v6  ·  高效一致性運行架構
%% ==========================================================================
%% v5 → v6 六項核心效率優化：
%%
%%  [E1] 邊界修正：SKILL_EVENTS 不再直接注入 ORG_EVENT_BUS
%%       → 改走 IER，由 IER 路由至 ORG_EVENT_BUS（消除 VS3→VS4 隱性侵入）
%%
%%  [E2] 邊界修正：ORG_AGG → VS5 粗粒度依賴
%%       → 改為 ORG_AGG 發射 OrgContextProvisioned 事件 → IER → VS5 ACL 防腐層接收
%%
%%  [E3] 事件管道統一：NOTIF_ROUTER 不再直接消費 ORG_EVENT_BUS
%%       → 改由 IER 路由 ScheduleAssigned → VS7，保持統一事件出口原則
%%
%%  [E4] Observability 橫切面擴展：
%%       → CBG_ENTRY（入口）+ IER（事件層）+ FUNNEL（投影層）全面掛載
%%       → 所有切片 Command Handler 統一由 CBG_ENTRY 繼承 TraceID，不需個別掛載
%%
%%  [E5] Outbox 唯一發送路徑：
%%       → WS_OUTBOX 為唯一 IER 投遞來源；WS_EVENT_BUS 僅供切片內部訂閱（in-process）
%%       → 消除 WS_EVENT_BUS → IER 的重複路徑，防止雙重發送
%%
%%  [E6] Claims 刷新單一觸發點：
%%       → Custom Claims 刷新僅由 VS1 Identity Slice 負責（登入/Token 刷新）
%%       → VS2 帳號治理 RoleChanged/PolicyChanged → IER → Claims Refresh Handler
%%       → 消除 VS2 直接寫入 Custom Claims 的隱性雙寫
%%
%% 閱讀順序：
