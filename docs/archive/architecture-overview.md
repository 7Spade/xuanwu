...
| VS4 Account Layer (Organization sub-type) | `features/account-organization.*` 切片 |
| GW Command Gateway | `features/infra.gateway-command/` |
| GW Query Gateway | `features/infra.gateway-query/` |
| GW Integration Event Router (IER) | `features/infra.event-router/` |
| GW OUTBOX_RELAY_WORKER [R1] | `features/infra.outbox-relay/` |
| GW DLQ 三級策略 [R5] | `features/infra.dlq-manager/` |
| VS5 Workspace Container — Application | `features/workspace-application/` 切片 |
| VS5 Workspace Container — Core | `features/workspace-core/`、`workspace-core.*` 切片 |
| VS5 Workspace Container — Governance | `features/workspace-governance.*` 切片 |
